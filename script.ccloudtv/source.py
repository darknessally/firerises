...
class Database(object):
        ...
        while True:
            ...
            try:
                ...
                if cancel_requested_callback is None:
                    xbmc.log('[script.ccloudtv] Database is locked, bailing out...', xbmc.LOGDEBUG)
                    break
                else:  # ignore 'database is locked'
                    xbmc.log('[script.ccloudtv] Database is locked, retrying...', xbmc.LOGDEBUG)

            except sqlite3.DatabaseError:
                self.conn = None
                if self.alreadyTriedUnlinking:
                    xbmc.log('[script.ccloudtv] Database is broken and unlink() failed', xbmc.LOGDEBUG)
                    break
                else:
                    try:
    ...
    def _updateChannelAndProgramListCaches(self, date, progress_callback, clearExistingProgramList):
        ...
        c = self.conn.cursor()
        try:
            xbmc.log('[script.ccloudtv] Updating caches...', xbmc.LOGDEBUG)
...
class XMLTVSource(Source):
    ...
    TYPE_cCloud_ALL = 0
    TYPE_cCloud_BASIC = 1
    TYPE_cCloud_UKBASIC = 2
    TYPE_cCloud_UKSKY = 3
    TYPE_cCloud_USTV = 4
    TYPE_cCloud_USUKBASIC = 5
    TYPE_CUSTOM = 6
    INTERVAL_ALWAYS = 0
    INTERVAL_12 = 1
    INTERVAL_24 = 2
    INTERVAL_48 = 3
    LOGO_SOURCE_CCLOUD = 0
    ...
    def __init__(self, addon):
        ...
        if (self.logoSource == XMLTVSource.LOGO_SOURCE_CCLOUD):
            self.logoFolder = 'special://home/addons/script.ccloudtv/resources/logos/'
        else:
            self.logoFolder = str(addon.getSetting('logos.folder'))

        if (self.xmltvType == XMLTVSource.TYPE_cCloud_ALL):
            self.xmltvFile = self.updateLocalFile(XMLTVSource.cCloud_ALL)
        elif (self.xmltvType == XMLTVSource.TYPE_cCloud_BASIC):
            self.xmltvFile = self.updateLocalFile(XMLTVSource.cCloud_BASIC)
        elif (self.xmltvType == XMLTVSource.TYPE_cCloud_UKBASIC):
            self.xmltvFile = self.updateLocalFile(XMLTVSource.cCloud_UKBASIC)
        elif (self.xmltvType == XMLTVSource.TYPE_cCloud_UKSKY):
            self.xmltvFile = self.updateLocalFile(XMLTVSource.cCloud_UKSKY)
        elif (self.xmltvType == XMLTVSource.TYPE_cCloud_USTV):
            self.xmltvFile = self.updateLocalFile(XMLTVSource.cCloud_USTV)
        elif (self.xmltvType == XMLTVSource.TYPE_cCloud_USUKBASIC):
            self.xmltvFile = self.updateLocalFile(XMLTVSource.cCloud_USUKBASIC)
        ...
        if (self.addonsType == XMLTVSource.INI_TYPE_CCLOUD):
    ...
    def updateLocalFile(self, name):
        ...
        if (not fetchFile and self.xmltvInterval <> XMLTVSource.INTERVAL_ALWAYS):
            ...
            if ((self.xmltvInterval == XMLTVSource.INTERVAL_12 and diff >= 43200) or
                ...
                fetchFile = True
                xbmc.log('[script.ccloudtv] Interval reached, fetching remote file...', xbmc.LOGDEBUG)
        else:
            fetchFile = True
        ...
        if (fetchFile):
            ...
            xbmc.log('[script.ccloudtv] Remote file fetching not due yet...', xbmc.LOGDEBUG)
        return path
    ...
    def parseXMLTVDate(self, origDateString):
        if origDateString.find(' ') != -1:
            ...
            xbmc.log('[script.ccloudtv] Date to normalize: ' + dateString, xbmc.LOGDEBUG)
            try:
            ...
            t = t + td_local
            xbmc.log('[script.ccloudtv] Import Time adjusted from: ' + str(t_tmp) + ' to: ' + str(t), xbmc.LOGDEBUG)
            return t
